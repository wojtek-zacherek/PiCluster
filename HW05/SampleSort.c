...
// --mca btl_vader_single_copy_mechanism none -np 3
...
int main(int argc, char *argv[]){
    ...
    if(my_rank == 0){
        ...
        for(i = 0; i < numBins; i++){
            arraysBinned[i] = (float *)malloc((1 + padding) * elementsPer * sizeof(float));
        }
    ...
    localBucket = (float *)malloc((1 + padding) * elementsPer * sizeof(float));
