...
void generateFractal(int my_rank, int comm_sz, int xRes, int yRes, int iter, double xMin, double xMax, double yMin, double yMax, double rMax, int choice);
int Mandelbrot(double C_R, double C_I, int maxInt, double rMax, int choice);
...
int main(int argc, char *argv[])
{
    ...
    generateFractal(my_rank, comm_sz, xRes, yRes, iter, xMin, xMax, yMin, yMax, rMax, choice);
...
void generateFractal(int my_rank, int comm_sz, int xRes, int yRes, int iter, double xMin, double xMax, double yMin, double yMax, double rMax, int choice)
{
    ...
    for (int i = 0; i < (xEnd - xStart); i++)
    {
        ...
        for (int j = 0; j < (yEnd - yStart); j++)
        {
            ...
            // if(my_rank == 0)
            //     printf("X,Y = %.2f,%.2f\n",tempX,tempY);
            val = Mandelbrot(tempX, tempY, iter, rMax, choice);
            submatrix[i][j] = val;
            // sem_wait(&mutex);
...
void makeColourfull(char **target, int **matrix, int xResolution, int yResolution, int iter)
{
    ...
    colorSelected.selection = redwhite;
    colorSelected.numEntries = 511;
    colorSelected.selection = colors;
    colorSelected.numEntries = iter;
    colorSelected.selection = bluewhite;
    colorSelected.numEntries = 255;
    // colorSelected.selection = blue; colorSelected.numEntries = 255;
    colorSelected.selection = blackredorangewhite;
    colorSelected.numEntries = 511;
